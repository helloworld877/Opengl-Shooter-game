...
bool our::ShaderProgram::attach(const std::string &filename, GLenum type) const
{
    ...
    // attach shader code to object
    glShaderSource(shaderObj, 1, &sourceCStr, NULL);
    // compile shader
    glCompileShader(shaderObj);

    // check on shader compilation errors
    std::string check = checkForShaderCompilationErrors(shaderObj);
    ...
    glAttachShader(this->program, shaderObj);

    // delete shader object as we don't need it anymore
...
bool our::ShaderProgram::link() const
{
    ...
    // check on linking errors
    std::string check = checkForLinkingErrors(this->program);
    if (check.length() != 0)
    {
        // print debug info on error
